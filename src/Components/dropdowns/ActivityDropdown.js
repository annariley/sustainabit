...
const ActivityDropdown = ({ value, setValue }) => {
  const [open, setOpen] = useState(false);
  const [items, setItems] = useState([
    {label: 'Commute', value: 'commute', icon: () => <Icon name="bike" size={18} color="#50692D" />},
    {label: 'Vegan Meal', value: 'veganMeal', icon: () => <Icon name="bowl-mix-outline" size={18} color="#50692D" />},
    {label: 'Custom', value: 'custom', icon: () => <Icon name="flower-poppy" size={18} color="#50692D" />},
  ]);
